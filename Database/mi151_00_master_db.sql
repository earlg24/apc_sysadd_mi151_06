...
CREATE TABLE IF NOT EXISTS `mi151_00_master_db`.`housekeeping_log` (
  ...
  `employee_id` INT NOT NULL,
  `housekeeping_log_status` ENUM('Completed', 'Not yet done') NULL,
  `housekeeping_log_timein` DATETIME NULL,
  `housekeeping_log_timeout` DATETIME NULL,
  `cleaning_status` ENUM('Completed', 'Not yet done') NULL,
  `inspected_by_employee_id` INT NULL,
  `inspection_status` ENUM('Completed', 'Not yet done') NULL,
  PRIMARY KEY (`id`, `room_id`, `room_room_type_id`, `employee_id`),
...
CREATE TABLE IF NOT EXISTS `mi151_00_master_db`.`housekeeping_log_details` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `housekeeping_log_details_checklist` VARCHAR(60) NULL,
  `housekeeping_log_details_status` ENUM('Completed', 'Not yet done') NULL,
  `housekeeping_log_details_timecompleted` DATETIME NULL,
  `housekeeping_log_id` INT NOT NULL,
  `housekeeping_log_room_id` INT NOT NULL,
  `housekeeping_log_room_room_type_id` INT NOT NULL,
  `housekeeping_log_employee_id` INT NOT NULL,
  PRIMARY KEY (`id`, `housekeeping_log_id`, `housekeeping_log_room_id`, `housekeeping_log_room_room_type_id`, `housekeeping_log_employee_id`),
  INDEX `fk_housekeeping_log_details_housekeeping_log1_idx` (`housekeeping_log_id` ASC, `housekeeping_log_room_id` ASC, `housekeeping_log_room_room_type_id` ASC, `housekeeping_log_employee_id` ASC),
  CONSTRAINT `fk_housekeeping_log_details_housekeeping_log1`
    FOREIGN KEY (`housekeeping_log_id` , `housekeeping_log_room_id` , `housekeeping_log_room_room_type_id` , `housekeeping_log_employee_id`)
    REFERENCES `mi151_00_master_db`.`housekeeping_log` (`id` , `room_id` , `room_room_type_id` , `employee_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
